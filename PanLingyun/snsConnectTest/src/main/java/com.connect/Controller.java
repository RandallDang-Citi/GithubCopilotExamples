...
public class Controller {
    ...
    @Autowired
    private ClassKetValueConfig classKetValueConfig;
    ...
    //将list转成map
    public static Map<String, String> listToMapJson(List<ClassKetValue> list) {
        Map<String, String> map = new HashMap<>();
        for (ClassKetValue classKetValue : list) {
            map.put(classKetValue.getJson(), classKetValue.getXml());
        }
        return map;
    }

    public static Map<String, String> listToMapXml(List<ClassKetValue> list) {
        Map<String, String> map = new HashMap<>();
        for (ClassKetValue classKetValue : list) {
            map.put(classKetValue.getXml(), classKetValue.getJson());
        }
        return map;
    }
    ...
    public String jsonToXMl(String value) throws IOException {
        String str = "";
        if(isXml(value)){
            Map<String, String> xmlToJsonFieldNameMappings = listToMapJson(classKetValueConfig.getList());
            str = convertXmlToJson(value, xmlToJsonFieldNameMappings);
        }
        if (isJson(value)){
            Map<String, String> jsonToXmlFieldNameMappings = listToMapXml(classKetValueConfig.getList());
